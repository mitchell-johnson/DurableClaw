...
describe("workspace interface", () => {
  ...
  it("opens connections and account security from mobile navigation in a native session", async () => {
    compact = true;
    vi.stubGlobal("fetch", async (path: string) =>
      Response.json(
        path === "/api/auth/options"
  ...
  it("dismisses the mobile drawer to show a native logout failure and allows retry", async () => {
    compact = true;
    let logoutFails = true;
    vi.stubGlobal("fetch", async (path: string) =>
      path === "/api/auth/sign-out" && logoutFails
        ? new Response(null, { status: 503 })
        : Response.json(
            path === "/api/auth/options"
              ? { enabled: true, accessRecovery: false }
