...
Model IDs are supplied through `CHAT_MODEL`, `BACKGROUND_MODEL`, and `BATCH_MODEL`. The synchronous transport uses OpenAI-compatible chat completions at OpenRouter. Set `OPENROUTER_PROVIDER` to one provider slug to restrict every new language-model request to that provider with fallbacks disabled. For Gemini 3.8 Flash, set all three models to `google/gemini-3.8-flash`, the provider to `google-ai-studio`, and `BACKGROUND_REASONING_EFFORT` to `high`. The effort defaults to `max` when omitted; explicit per-request reasoning remains unchanged. See OpenRouter's [provider routing](https://openrouter.ai/docs/guides/routing/provider-selection) and [reasoning options](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens).

Without a provider pin, housekeeping uses OpenRouter's asynchronous Batch API. The [Batch API does not support provider preferences](https://openrouter.ai/docs/batch-quickstart), so pinned deployments run queued housekeeping through synchronous inference inside their Durable Object, one task per alarm pass with a 30-second request timeout and 2 MiB input/output limits. The outbox saves the attempt and recovery deadline before dispatch, then saves completed text before applying it. Failed local writes reuse saved text; interrupted inference may incur another model charge within the existing three-attempt limit. Previously submitted batches can still be polled after enabling a pin. Embeddings and optional reranking continue to use Workers AI.
