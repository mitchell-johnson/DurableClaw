...
`run_device_bash` previews the device ID, command, absolute working directory and timeout. The existing server-issued confirmation is bound to those exact arguments and the conversation, expires after ten minutes, and is consumed once. The authenticated web UI or single-use buttons in the owner’s linked Telegram chat can approve the complete preview. Messaging approval checks the current owner, link, private sender/chat, provider message, opaque token, and expiry; ordinary chat messages cannot approve. A queued command expires after five minutes. Current device revocation, owner authority and tool policy are checked again before claim. A failed or absent tool-policy check never dispatches a command.
